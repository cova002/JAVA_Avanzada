...
public class AppController {
 ...
 static List<Persona> listaPersonas = new ArrayList<Persona>();
  ...
  @PutMapping("/{id}")
  public void actualizar(@PathVariable("id") String id, @RequestBody Persona persona) {
    listaPersonas.remove(new Persona(id));
    listaPersonas.add(persona);
  }
      @PutMapping("/edit_persona/{id}")
  Persona replaceEmployee(@RequestBody Persona newPersona, @PathVariable int id) {
    
    return repository.findById(id)
      .map(employee -> {
        newPersona.setNombre("Fer");
        newPersona.setApellido("Cova");
